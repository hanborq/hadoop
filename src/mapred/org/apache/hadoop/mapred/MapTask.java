...
class MapTask extends Task {
  ...
  private TaskSplitInfo taskSplitInfo;
  private final static int APPROX_HEADER_LENGTH = 150;
  private static byte[] kvbuffer;
  private static int[] kvoffsets;     // indices into kvindices
  private static int[] kvindices;     // partition, k/v offsets into kvbuffer
  
  public static void initKvBuffers(JobConf job) {
      ...
      int kvBufferLength = maxMemUsage - recordCapacity;
      
      if(kvbuffer == null || kvBufferLength != kvbuffer.length)
    	  kvbuffer = new byte[kvBufferLength];
      
      recordCapacity /= RECSIZE;
      
      int kvoffsetsLength = recordCapacity;
      int kvindicesLength = recordCapacity * 3;
      
      if(kvoffsets == null || kvoffsets.length != kvoffsetsLength)
    	  kvoffsets = new int[kvoffsetsLength];
      
      if(kvindices == null || kvindices.length != kvindicesLength)
    	  kvindices = new int[kvindicesLength];
  }
  ...
  public MapTask(String jobFile, TaskAttemptID taskId, 
                 int partition, TaskSplitInfo splitInfo,
                 int numSlotsRequired) {
    super(jobFile, taskId, partition, numSlotsRequired);
    this.taskSplitInfo = splitInfo;
    Merger.includeFinalMerge = true;
  }
  ...
  public void readFields(DataInput in) throws IOException {
    super.readFields(in);
    if (isMapOrReduce()) {
    	boolean hasTaskSplitInfo = in.readBoolean();
   
    	if(hasTaskSplitInfo) {
    		taskSplitInfo = new TaskSplitInfo();
    		taskSplitInfo.setConf(conf);
    		taskSplitInfo.readFields(in);
    	}
    	else
    		taskSplitInfo = null;
    }
  ...
  public void run(final JobConf job, final TaskUmbilicalProtocol umbilical) 
    ...
    if (useNewApi) {
      runNewMapper(job, taskSplitInfo, umbilical, reporter);
    } else {
      runOldMapper(job, taskSplitInfo, umbilical, reporter);
    }
  ...
  void runOldMapper(final JobConf job,
                    final TaskSplitInfo splitInfo,
                    final TaskUmbilicalProtocol umbilical,
                    TaskReporter reporter
                    ) throws IOException, InterruptedException,
                             ClassNotFoundException {
    InputSplit inputSplit = splitInfo.getOldSplit();
  ...
  implements MapOutputCollector<K, V>, IndexedSortable {
    ...
    private static final int ACCTSIZE = 3;  // total #fields in astcct
    ...
    public MapOutputBuffer(TaskUmbilicalProtocol umbilical, JobConf job,
      ...
      avoidsort = job.getBoolean("mapred.sort.avoidance", false);
      LOG.info("Map avoid sort = " + avoidsort);
      ...
      int kvoffsetsLength = recordCapacity;
      int kvindicesLength = recordCapacity * ACCTSIZE;
    ...
    private void sortAndSpill() throws IOException, ClassNotFoundException,
      ...
      try {
        ...
        final int endPosition = (kvend > kvstart) ? kvend : kvoffsets.length
            + kvend;
        if (avoidsort) {
    ...
    private void sortPartitionsOnly(final int l, final int r) {
      new IndexedCountingSortable(oldKvCounts, r - l) {
        @Override
    ...
    private void mergeParts() throws IOException, InterruptedException, 
      ...
      }
      {
        ...
        for (int parts = 0; parts < partitions; parts++) {
          ...
          if (avoidsort) {
            writer = new Writer<K, V>(job, finalOut, null, null, codec,
                spilledRecordsCounter);
            ConcatenateMerger.writeFile(writer, segmentList, reporter, null,
                job);
          } else {
            int mergeFactor = job.getInt("io.sort.factor", 100);
            // sort the segments only if there are intermediate
            // merges
            boolean sortSegments = segmentList.size() > mergeFactor;
            // merge
            @SuppressWarnings("unchecked")
            RawKeyValueIterator kvIter = Merger.merge(job, rfs, keyClass,
                valClass, codec, segmentList, mergeFactor, new Path(mapId
                    .toString()), job.getOutputKeyComparator(), reporter,
                sortSegments, null, spilledRecordsCounter, null);

            // write merged output to disk
            writer = new Writer<K, V>(job, finalOut, keyClass, valClass,
