...
class Child {
	...
	/**
	 * Return true if running in a task/child JVM. This should only be used for
	...
	public static void main(String[] args) throws Throwable {
		...
		boolean autoGC = defaultConf.getBoolean("child.jvm.auto.gc", true);

		String host = args[0];
		int port = Integer.parseInt(args[1]);
		final InetSocketAddress address = new InetSocketAddress(host, port);
		final boolean isMap = Integer.parseInt(args[2]) == 1;

		if (isMap) {
			MapTask.initKvBuffers(defaultConf);
		}

		final String logLocation = args[3];
		final int SLEEP_LONGER_COUNT = 5;
		int jvmIdInt = Integer.parseInt(args[4]);
		JVMId jvmId = new JVMId(isMap, jvmIdInt);
		JobID lastJobID = null;
		TaskAttemptID lastTaskID = null;
		 ...
		 * Credentials credentials = TokenCache.loadTokens(jobTokenFile,
		 * defaultConf); LOG.debug("loading token. # keys ="
		 * +credentials.numberOfSecretKeys() + "; from file=" + jobTokenFile);
		 * 
		...
		final TaskUmbilicalProtocol umbilical = current
				.doAs(new PrivilegedExceptionAction<TaskUmbilicalProtocol>() {
					@Override
					public TaskUmbilicalProtocol run() throws Exception {
						return (TaskUmbilicalProtocol) RPC.getProxy(
			...
			while (true) {
				...
				if (myTask.shouldDie()) {
					...
					if (myTask.getTask() == null) {
						...
						if (autoGC
								&& System.currentTimeMillis() - lastGCTime > 60000
								&& numTasksExecuted > 10) {
							System.gc();
				...
				// Create the JobConf and determine if this job gets segmented
				// task logs
				final JobConf job = (JobConf) task.getConf();
				...
				// forcefully turn off caching for localfs. All cached
				// FileSystems
				...
				task.setJobTokenSecret(JobTokenSecretManager
						.createSecretKey(new byte[] { 1 }));

				// setup the child's mapred-local-dir. The child is now
				// sandboxed and
				...
				if (lastJobID == null || !task.getJobID().equals(lastJobID)) {
					String classPath = job.get("user.class.path");
					String classPathItems[] = classPath.split(System
							.getProperty("path.separator"));

					URL urls[] = new URL[classPathItems.length];

					for (int i = 0; i < urls.length; ++i) {
						if (classPathItems[i].endsWith(".jar") == false
								&& classPathItems[i].endsWith("/") == false) {
							classPathItems[i] = classPathItems[i] + "/";
						}

						urls[i] = new URL("file", "localhost",
								classPathItems[i]);
					}
					...
					defaultConf.setClassLoader(urlClassLoader);
				}

				job.setClassLoader(Thread.currentThread()
						.getContextClassLoader());

				lastJobID = task.getJobID();
				// setupWorkDir actually sets up the symlinks for the
				// distributed
				// cache. After a task exits we wipe the workdir clean, and
				// hence
				...
				LOG.debug("Creating remote user to execute task: "
						+ job.get("user.name"));
				childUGI = UserGroupInformation.createRemoteUser(job
						.get("user.name"));
				// Add tokens to new user so that it may execute its task
				// correctly.
				for (Token<?> token : UserGroupInformation.getCurrentUser()
						.getTokens()) {
				...
				childUGI.doAs(new PrivilegedExceptionAction<Object>() {
					...
					public Object run() throws Exception {
						try {
							...
							FileSystem.get(job).setWorkingDirectory(
									job.getWorkingDirectory());
							taskFinal.run(job, umbilical); // run the task
						} finally {
							TaskLog.syncLogs(logLocation, taskid, isCleanup,
									logIsSegmented(job));
							TaskLogsTruncater trunc = new TaskLogsTruncater(
									defaultConf);
							trunc.truncateLogs(new JVMInfo(TaskLog
									.getAttemptDir(taskFinal.getTaskID(),
											taskFinal.isTaskCleanupTask()),
	...
	static void localizeTask(Task task, JobConf jobConf, String logLocation)
		...
		/*
		 * //write the localized task jobconf LocalDirAllocator lDirAlloc = new
		 * LocalDirAllocator(JobConf.MAPRED_LOCAL_DIR_PROPERTY); Path
		 * localTaskFile = lDirAlloc.getLocalPathForWrite(TaskTracker.JOBFILE,
		 * jobConf); LOG.info("local job file=" + localTaskFile.toString());
		 * JobLocalizer.writeLocalJobFile(localTaskFile, jobConf);
		 * task.setJobFile(localTaskFile.toString()); task.setConf(jobConf);
		 */
	}
}
