...
class Child {
  ...
  public static void main(String[] args) throws Throwable {
    ...
    final TaskUmbilicalProtocol umbilical = 
      current.doAs(new PrivilegedExceptionAction<TaskUmbilicalProtocol>() {
        @Override
    ...
    try {
      while (true) {
        ...
        if(lastJobID == null || !task.getJobID().equals(lastJobID)) { 
	        ...
	        String classPathItems[] = classPath.split(System.getProperty("path.separator"));
	        
	        URL urls[] = new URL[classPathItems.length];
	        
	        for(int i = 0; i < urls.length; ++i) {
	        	if(classPathItems[i].endsWith(".jar") == false && classPathItems[i].endsWith("/") == false) {
	        		classPathItems[i] = classPathItems[i] + "/";
	        	}
	        	
	        	urls[i] = new URL("file", "localhost", classPathItems[i]);
	        }
	        URLClassLoader urlClassLoader = new URLClassLoader(urls, defaultClassLoader);
  ...
  throws IOException{
    ...
    LOG.info("local job file=" + localTaskFile.toString());
