...
public class MergeManager<K, V>
{
  ...
  // when avoidsort = true, only one merger thread : in memory merge thread.
    ...
    public MergeManager(TaskAttemptID reduceId, JobConf jobConf, FileSystem localFS,
            LocalDirAllocator localDirAllocator, Reporter reporter, CompressionCodec codec,
            Class<? extends Reducer> combinerClass, CombineOutputCollector<K, V> combineCollector,
            ...
            Progress mergePhase, MapOutputFile mapOutputFile)
    {
        ...
        this.onDiskMapOutputs = new TreeSet<MapOutput<K, V>>(new MapOutputComparator<K, V>());

        final float maxInMemCopyUse = jobConf.getFloat("mapred.job.shuffle.input.buffer.percent",
                0.90f);
        ...
        this.memoryLimit = (long) (jobConf.getLong("mapred.reduce.memory.totalbytes", Math.min(
                Runtime.getRuntime().maxMemory(), Integer.MAX_VALUE)) * maxInMemCopyUse);
        ...
        this.memToMemMergeOutputsThreshold = jobConf.getInt("mapred.inmem.merge.threshold",
                ioSortFactor);
        ...
        LOG.info("MergerManager: memoryLimit=" + memoryLimit + ", " + "maxSingleShuffleLimit="
                ...
                + "ioSortFactor=" + ioSortFactor + ", " + "memToMemMergeOutputsThreshold="
                + memToMemMergeOutputsThreshold);

        boolean allowMemToMemMerge = jobConf.getBoolean("mapred.inmem.merge.threshold.enabled",
                false);
            ...
            throws IOException
    {
        if (!canShuffleToMemory(requestedSize))
        {
            ...
            return new MapOutput<K, V>(mapId, this, requestedSize, jobConf, localDirAllocator,
        ...
        LOG.debug(mapId + ": Proceeding with shuffle since usedMemory (" + usedMemory
     ...
     * @return
     */
    private synchronized MapOutput<K, V> unconditionalReserve(TaskAttemptID mapId,
  ...
  public synchronized void closeOnDiskFile(MapOutput<K, V> file) {
    ...
    public synchronized RawKeyValueIterator close() throws Throwable
    {
        ...
        {
          List<MapOutput<K, V>> memory = new ArrayList<MapOutput<K, V>>(inMemoryMergedMapOutputs);
    ...
    private class IntermediateMemoryToMemoryMerger extends MergeThread<MapOutput<K, V>, K, V>
    {
        ...
        public void merge(List<MapOutput<K, V>> inputs) throws IOException
        {
            ...
            MapOutput<K, V> mergedMapOutputs = unconditionalReserve(dummyMapId, mergeOutputSize,
                    false);

            Writer<K, V> writer = new InMemoryWriter<K, V>(mergedMapOutputs.getArrayStream());
            if (!avoidsort)
            {
                LOG.info("Initiating sorted Memory-to-Memory merge with " + noInMemorySegments
                        + " segments of total-size: " + mergeOutputSize);

                RawKeyValueIterator rIter = Merger.merge(jobConf, rfs, (Class<K>) jobConf
                        .getMapOutputKeyClass(), (Class<V>) jobConf.getMapOutputValueClass(),
                        inMemorySegments, inMemorySegments.size(), new Path(reduceId.toString()),
                        (RawComparator<K>) jobConf.getOutputKeyComparator(), reporter, null, null,
                        null);
                Merger.writeFile(rIter, writer, reporter, jobConf);
            }
            else
            {
                LOG.info("Initiating unsorted Memory-to-Memory merge with " + noInMemorySegments
                        + " segments of total-size: " + mergeOutputSize);
                ConcatenateMerger.writeFile(writer, inMemorySegments, reporter, null, jobConf);
            }

            writer.close();

            LOG.info(reduceId + " Memory-to-Memory merge of the " + noInMemorySegments
    ...
    private class InMemoryMerger extends MergeThread<MapOutput<K, V>, K, V>
    {
        ...
        public void merge(List<MapOutput<K, V>> inputs) throws IOException
        {
            ...
            Path outputPath = mapOutputFile.getInputFileForWrite(mapTaskId, mergeOutputSize)
            ...
            try
            {
                if (!avoidsort)
                {
                    Writer<K, V> writer = new Writer<K, V>(jobConf, rfs, outputPath,
                            (Class<K>) jobConf.getMapOutputKeyClass(), (Class<V>) jobConf
                                    .getMapOutputValueClass(), codec, null);

                    LOG.info("Initiating sorted in-memory merge with " + noInMemorySegments
                            + " segments...");
                    RawKeyValueIterator rIter = null;
                    rIter = Merger.merge(jobConf, rfs, (Class<K>) jobConf.getMapOutputKeyClass(),
                ...
                {
                    LOG.info("Initiating unsorted in-memory  merge with " + noInMemorySegments
                            + " segments...");
                    Writer<K, V> writer = new Writer<K, V>(jobConf, rfs, outputPath, null, null,
                            codec, null);
        ...
        public void merge(List<MapOutput<K, V>> inputs) throws IOException
        {
            ...
            try
            {
                if (!avoidsort)
                {
                    ...
                    iter = Merger.merge(jobConf, rfs, (Class<K>) jobConf.getMapOutputKeyClass(),
                            (Class<V>) jobConf.getMapOutputValueClass(), codec, tmpInputs, true,
                            ioSortFactor, tmpDir, (RawComparator<K>) jobConf
                                    .getOutputKeyComparator(), reporter, spilledRecordsCounter,
                            null, mergedMapOutputsCounter, null);
                ...
                else
                {
                    ...
                    for (MapOutput<K, V> file : inputs)
                    {
                        ...
                        segments.add(new Segment<K, V>(jobConf, rfs, tmp, 0, rfs.getFileStatus(tmp)
                                .getLen(), file.getSize(), codec, false, (file.toString().endsWith(
                                Task.MERGED_OUTPUT_PREFIX) ? null : mergedMapOutputsCounter)));
                    }

                    ConcatenateMerger.writeFile(writer, segments, reporter, spilledRecordsCounter,
                            jobConf);
                    writer.close();
                    uncompressedSize = writer.getRawLength();
            ...
            throws IOException
    {
        ...
        try
        {
            CombineValuesIterator values = new CombineValuesIterator(kvIter, comparator, keyClass,
                    valClass, job, Reporter.NULL, inCounter);
            while (values.more())
            {
            ...
            List<Segment<K, V>> inMemorySegments, long leaveBytes) throws IOException
    {
        ...
        while (fullSize > leaveBytes)
        {
            ...
            totalSize += size;
            fullSize -= size;
            Reader<K, V> reader = new InMemoryReader<K, V>(MergeManager.this, mo.getMapId(), data,
                    0, (int) size);
            ...
            throws IOException
    {
        LOG.info("unsorted finalMerge called with " + inMemoryMapOutputs.size()
                + " in-memory map-outputs and " + onDiskMapOutputs.size() + " on-disk map-outputs");

        // memory percentage to store reduce input.
        final float maxRedPer = job.getFloat("mapred.job.reduce.input.buffer.percent", 0f);
        if (maxRedPer > 1.0 || maxRedPer < 0.0)
        {
            throw new IOException("mapred.job.reduce.input.buffer.percent" + maxRedPer);
        }
        int maxInMemReduce = (int) Math.min(Runtime.getRuntime().maxMemory() * maxRedPer,
                Integer.MAX_VALUE);
        ...
        if (inMemoryMapOutputs.size() > 0)
        {
            ...
            inMemToDiskBytes = createInMemorySegments(inMemoryMapOutputs, memDiskSegments,
                    maxInMemReduce);
            ...
            if (numMemDiskSegments > 0)
            {
                final Path outputPath = mapOutputFile.getInputFileForWrite(mapId, inMemToDiskBytes)
                        .suffix(Task.MERGED_OUTPUT_PREFIX);
                Writer<K, V> writer = new Writer<K, V>(jobConf, rfs, outputPath, null, null, codec,
                        null);
        ...
        for (Path file : onDisk)
        {
            ...
            finalSegments.add(new Segment<K, V>(job, fs, file, codec, job.getKeepFailedTaskFiles(),
        ...
        return new UnsortedKVIterator(job, fs, finalSegments, reporter, false).merge(
                spilledRecordsCounter, null, null);
            ...
            throws IOException
    {
        ...
        if (inMemoryMapOutputs.size() > 0)
        {
            ...
            if (numMemDiskSegments > 0 && ioSortFactor > onDiskMapOutputs.size())
            {
                ...
                final RawKeyValueIterator rIter = Merger.merge(job, fs, keyClass, valueClass,
                        ...
                        spilledRecordsCounter, null, mergePhase);
                final Writer<K, V> writer = new Writer<K, V>(job, fs, outputPath, keyClass,
        ...
        for (Path file : onDisk)
        {
            onDiskBytes += fs.getFileStatus(file).getLen();
            LOG.debug("Disk file: " + file + " Length is " + fs.getFileStatus(file).getLen());
            diskSegments.add(new Segment<K, V>(job, fs, file, codec, keepInputs, (file.toString()
                    .endsWith(Task.MERGED_OUTPUT_PREFIX) ? null : mergedMapOutputsCounter)));
        }
        LOG.info("Merging " + onDisk.length + " files, " + onDiskBytes + " bytes from disk");
        ...
        if (0 != onDiskBytes)
        {
            ...
            RawKeyValueIterator diskMerge = Merger.merge(job, fs, keyClass, valueClass, codec,
                    ...
                    false, spilledRecordsCounter, null, thisPhase);
            diskSegments.clear();
            if (0 == finalSegments.size())
            {
                return diskMerge;
            }
            finalSegments.add(new Segment<K, V>(new RawKVIteratorReader(diskMerge, onDiskBytes),
                    true));
        }
