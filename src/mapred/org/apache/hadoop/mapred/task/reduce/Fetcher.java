...
class Fetcher<K, V> extends Thread
{
    ...
    public Fetcher(JobConf job, TaskAttemptID reduceId, ShuffleScheduler<K, V> scheduler,
            ...
            ExceptionReporter exceptionReporter, SecretKey jobTokenSecret)
    {
        ...
        wrongLengthErrs = reporter.getCounter(SHUFFLE_ERR_GRP_NAME, ShuffleErrors.WRONG_LENGTH
                .toString());
        badIdErrs = reporter.getCounter(SHUFFLE_ERR_GRP_NAME, ShuffleErrors.BAD_ID.toString());
        wrongMapErrs = reporter
                .getCounter(SHUFFLE_ERR_GRP_NAME, ShuffleErrors.WRONG_MAP.toString());
        connectionErrs = reporter.getCounter(SHUFFLE_ERR_GRP_NAME, ShuffleErrors.CONNECTION
                .toString());
        wrongReduceErrs = reporter.getCounter(SHUFFLE_ERR_GRP_NAME, ShuffleErrors.WRONG_REDUCE
                .toString());
        ...
        this.readTimeout = job
                .getInt("mapreduce.reduce.shuffle.read.timeout", DEFAULT_READ_TIMEOUT);
...
//            SecureShuffleUtils.verifyReply(replyHash, encHash, jobTokenSecret);
        ...
        {
            ioErrs.increment(1);
            LOG.warn("Failed to connect to " + host + " with " + remaining.size() + " map outputs",
                    ie);
    ...
    private boolean copyMapOutput(MapHost host, DataInputStream input, Set<TaskAttemptID> remaining)
    {
        ...
        catch (IOException ioe)
        {
            ...
            {
                LOG.info("fetcher#" + id + " failed to read map header" + mapId + " decomp: "
            ...
            Set<TaskAttemptID> remaining, TaskAttemptID mapId)
    {
        ...
        if (forReduce != reduce)
        {
            ...
            LOG.warn(getName() + " data for the wrong reduce map: " + mapId + " len: "
                    + compressedLength + " decomp len: " + decompressedLength + " for reduce "
                    + forReduce);
            return false;
        }
    ...
    private void shuffleToMemory(MapHost host, MapOutput<K, V> mapOutput, InputStream input,
    ...
    private void shuffleToDisk(MapHost host, MapOutput<K, V> mapOutput, InputStream input,
            long compressedLength) throws IOException
    {
        ...
        if (bytesLeft != 0)
        {
            throw new IOException("Incomplete map output received for " + mapOutput.getMapId()
                    + " from " + host.getHostName() + " (" + bytesLeft + " bytes missing of "
                    + compressedLength + ")");
        }
    }
}
