...
abstract class TaskRunner extends Thread {
  ...
  public final void run() {
    ...
    try {
      ...
      List<String> userClassPaths = getUserClassPaths(conf, workDir, taskDistributedCacheManager);
      
      conf.set("user.class.path", StringUtils.join(SYSTEM_PATH_SEPARATOR, userClassPaths));

      
      String jobTokenFile = conf.get(TokenCache.JOB_TOKENS_FILENAME);
      conf.set(UserGroupInformation.HADOOP_TOKEN_FILE_LOCATION, jobTokenFile);
  ...
  private List<String> getUserClassPaths(JobConf conf2, File workDir,
			TaskDistributedCacheManager taskDistributedCacheManager2) throws IOException {
    // Accumulates class paths for child.
