...
class LocalJobRunner implements JobSubmissionProtocol {
  ...
  private class Job extends Thread
    ...
    private JobSubmitInfo splitInfo;

    public long getProtocolVersion(String protocol, long clientVersion) {
      return TaskUmbilicalProtocol.versionID;
    }
    
    public Job(JobID jobid, String jobSubmitDir, JobConf conf, JobSubmitInfo splitInfo) throws IOException {
    ...
    public void run() {
      ...
      try {
        ...
        TaskSplitInfo taskSplitInfo[] = splitInfo.getTaskSplitInfos();
        
        for (int i = 0; i < taskSplitInfo.length; i++) {
  ...
  public JobStatus submitJob(JobID jobid, String jobSubmitDir, 
                             Credentials credentials, JobSubmitInfo splitInfo) 
  throws IOException {
    Job job = new Job(jobid, jobSubmitDir, conf, splitInfo);
