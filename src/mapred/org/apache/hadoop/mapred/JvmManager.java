...
class JvmManager {
  ...
  static class JvmManagerForType {
    ...
    LocalDirAllocator lDirAllocator = new LocalDirAllocator("mapred.local.dir");
    ReentrantLock jvmManagerLock = new ReentrantLock();
    ...
    public JvmManagerForType(int maxJvms, boolean isMap, 
        TaskTracker tracker) throws IOException {
    ...
    private JvmEnv getDefaultJvmEnv(JVMId id) throws IOException {
    	...
    	String jvmSubDir =  TaskTracker.SUBDIR + Path.SEPARATOR + id;
  	  	Path jvmDir = lDirAllocator.getLocalPathForWrite(jvmSubDir, tracker.getJobConf());
        ...
        Vector<String> vmArgs = getVMArgs(new File(workDir.toString()), logDir, conf);
        
        Path stdout = new Path(logDir, TaskLog.LogName.STDOUT.toString());
        Path stderr = new Path(logDir, TaskLog.LogName.STDERR.toString());
        
        
        return new JvmEnv(setupCmds, 
        		vmArgs, 
        		new File(stdout.toString()), 
        		new File(stderr.toString()),
        		TaskLog.getTaskLogLength(conf),
        		new File(workDir.toString()),
        		conf);
    }
    

	private Vector<String> getVMArgs(File workDir, Path logDir, JobConf conf) throws IOException {
		Path logFileName = new Path(logDir, TaskLog.LogName.SYSLOG.toString());
		List<String> classPaths = TaskRunner.getClassPaths(
				conf, new File(workDir.toString()), null);
		...
		//child opts
		String childOptStr = conf.get(JobConf.MAPRED_TASK_JAVA_OPTS, JobConf.DEFAULT_MAPRED_TASK_JAVA_OPTS);
	...
	public void setRunningTaskForJvm(JVMId jvmId, 
        TaskRunner t) {
    ...
    public TaskInProgress getTaskForJvm(JVMId jvmId)
    	...
    	synchronized(jvmRunner) {
    		TaskInProgress result = getTaskForJvmNoWait(jvmId);
    		jvmManagerLock.unlock();
    ...
    private void killJvmRunner(JvmRunner jvmRunner
                                            ) throws IOException,
                                                     InterruptedException {
      try {
      	jvmManagerLock.lock();
    ...
    private void removeJvm(JVMId jvmId) {
    	try {
    		jvmManagerLock.lock();
    ...
    private void spawnNewJvm(JobID jobId, JvmEnv env,  
        TaskRunner t) throws IOException {
      this.launchJvmWithTask(currentJvmId.getAndIncrement(), t);
    }
    
    private void updateOnJvmExit(JVMId jvmId, 
        int exitCode) {
