...
class ReduceTask extends Task {
  ...
  private boolean avoidsort = false;
  ...
  public ReduceTask(String jobFile, TaskAttemptID taskId,
    ...
    Merger.includeFinalMerge = false;
  ...
  public void run(JobConf job, final TaskUmbilicalProtocol umbilical)
    ...
    this.avoidsort = job.getBoolean("mapred.sort.avoidance", false);

    LOG.info(" avoid sort = " + avoidsort);
    ...
    if (!isLocal) {
      ...
      final Shuffle shuffle = new Shuffle(getTaskID(), job,
          ...
          failedShuffleCounter, mergedMapOutputsCounter, taskStatus,
          copyPhase, sortPhase, this, mapOutputFile, jvmContext);
      ...
      if(avoidsort)
      {
        ...
        shuffleThread.start();
        rIter = shuffle.getUnsortedKVIterator();
      }
      else
      {
        rIter = shuffle.run();
      ...
      rIter = Merger.merge(job, rfs, job.getMapOutputKeyClass(), job
          .getMapOutputValueClass(), codec, getMapFiles(rfs, true), !conf
          .getKeepFailedTaskFiles(), job.getInt("io.sort.factor", 100),
  ...
  void runOldReducer(JobConf job,
    ...
    try {
      ...
      ReduceValuesIterator<INKEY, INVALUE> values = isSkipping() ? new SkippingReduceValuesIterator<INKEY, INVALUE>(
          rIter, avoidsort ? null : comparator, keyClass, valueClass, job,
          reporter, umbilical)
          : new ReduceValuesIterator<INKEY, INVALUE>(rIter, avoidsort ? null
  ...
  void runNewReducer(JobConf job,
    ...
    reducerContext = createReduceContext(reducer, job, getTaskID(), rIter,
        reduceInputKeyCounter, reduceInputValueCounter, trackedRW, committer,
        reporter, avoidsort ? null : comparator, keyClass, valueClass);
