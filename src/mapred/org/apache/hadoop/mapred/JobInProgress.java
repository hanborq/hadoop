...
public class JobInProgress {
	...
	final Map<TaskType, Long> firstTaskLaunchTimes = new EnumMap<TaskType, Long>(
			TaskType.class);
	...
	public static enum Counter {
		NUM_FAILED_MAPS, NUM_FAILED_REDUCES, TOTAL_LAUNCHED_MAPS, TOTAL_LAUNCHED_REDUCES, OTHER_LOCAL_MAPS, DATA_LOCAL_MAPS, RACK_LOCAL_MAPS, SLOTS_MILLIS_MAPS, SLOTS_MILLIS_REDUCES, FALLOW_SLOTS_MILLIS_MAPS, FALLOW_SLOTS_MILLIS_REDUCES
	}
	...
	protected JobInProgress(JobID jobid, JobConf conf, JobTracker tracker) {
		...
		this.profile = new JobProfile(conf.getUser(), jobid, "", "", conf
	...
	JobInProgress(JobTracker jobtracker, final JobConf conf, JobInfo jobInfo,
			int rCount, Credentials ts, JobSubmitInfo splitInfo) throws IOException,
		...
		try {
			...
			fs = userUGI.doAs(new PrivilegedExceptionAction<FileSystem>() {
				public FileSystem run() throws IOException {
					return jobSubmitDir.getFileSystem(conf);
				}
			});
			 ...
			 * FileStatus fstatus = fs.getFileStatus(submitJobFile); if
			 * (fstatus.getLen() > jobtracker.MAX_JOBCONF_SIZE) { throw new
			 * IOException("Exceeded max jobconf size: " + fstatus.getLen() +
			 * " limit: " + jobtracker.MAX_JOBCONF_SIZE); }
			 * 
			 * this.localJobFile = conf.getLocalPath(JobTracker.SUBDIR +"/"+jobId +
			 * ".xml");
			 * 
			 * fs.copyToLocalFile(jobFilePath, localJobFile); conf = new
			 * JobConf(localJobFile);
			 */
			...
			if (!conf.getUser().equals(user)) {
				String desc = "The username " + conf.getUser() + " obtained from the "
						...
						+ "authenticated as";
				AuditLogger.logFailure(user, Operation.SUBMIT_JOB.name(), conf
						.getUser(), jobId.toString(), desc);
			...
			this.profile = new JobProfile(user, jobId, jobFile, url, conf
					.getJobName(), conf.getQueueName());
	...
	private Map<Node, List<TaskInProgress>> createCache(JobSubmitInfo submitInfo,
			int maxLevel) {
		Map<Node, List<TaskInProgress>> cache = new IdentityHashMap<Node, List<TaskInProgress>>(
				maxLevel);

		Set<String> uniqueHosts = new TreeSet<String>();
		for (int i = 0; i < submitInfo.getSplitNum(); i++) {
			String[] splitLocations = submitInfo.getSplitInfos()[i].getLocations();
		...
		if (localityWaitFactor == DEFAULT_LOCALITY_WAIT_FACTOR) {
			...
			if (clusterNodes > 0) {
				localityWaitFactor = Math.min((float) jobNodes / clusterNodes,
						localityWaitFactor);
			}
	...
	/**
	 * Set the number of slots required to run a single map task-attempt. This is
	 * typically set by schedulers which support high-ram jobs.
	 * 
	 * @param slots
	...
	/**
	 * Set the number of slots required to run a single reduce task-attempt. This
	 * is typically set by schedulers which support high-ram jobs.
	 * 
	 * @param slots
	 *          the number of slots required to run a single reduce task-attempt
	 */
	...
	public synchronized void initTasks() throws IOException,
		...
		//
		TaskSplitInfo[] taskSplits = splitInfo.getTaskSplitInfos();

		numMapTasks = taskSplits.length;
		...
		if (maxTasks > 0 && numMapTasks + numReduceTasks > maxTasks) {
			throw new IOException("The number of tasks for this job "
					+ (numMapTasks + numReduceTasks) + " exceeds the configured limit "
					+ maxTasks);
		}
		...
		for (int i = 0; i < numMapTasks; ++i) {
			inputLength += splitInfo.getSplitInfos()[i].getInputDataLength();
			maps[i] = new TaskInProgress(jobId, jobFile,
					splitInfo.getSplitInfos()[i], jobtracker, conf, this, i,
					numSlotsPerMap);
		}
		LOG.info("Input size for job " + jobId + " = " + inputLength
				+ ". Number of splits = " + taskSplits.length);

		// Set localityWaitFactor before creating cache
		localityWaitFactor = conf.getFloat(LOCALITY_WAIT_FACTOR,
				DEFAULT_LOCALITY_WAIT_FACTOR);
		if (numMapTasks > 0) {
			nonRunningMapCache = createCache(splitInfo, maxLevel);
		}
		...
		// split.
		cleanup[0] = new TaskInProgress(jobId, jobFile, new SplitInfo(), jobtracker, conf,
				this, numMapTasks, 1);
		...
		// split.
		setup[0] = new TaskInProgress(jobId, jobFile, new SplitInfo(), jobtracker, conf, this,
				numMapTasks + 1, 1);
	...
	TaskSplitMetaInfo[] createSplits(org.apache.hadoop.mapreduce.JobID jobId)
			throws IOException {
		TaskSplitMetaInfo[] allTaskSplitMetaInfo = SplitMetaInfoReader
				.readSplitMetaInfo(jobId, fs, jobtracker.getConf(), jobSubmitDir);
	 ...
	 * 
	 * @param type
	 *          {@link TaskType} of the tasks required
	 * @return An array of {@link TaskInProgress} matching the given type. Returns
	 *         an empty array if no tasks are found for the given type.
	 */
	...
	public synchronized void updateTaskStatus(TaskInProgress tip,
		...
		if (change) {
			...
			TaskTracker taskTracker = this.jobtracker.getTaskTracker(tip
					.machineWhereTaskRan(taskid));
			TaskTrackerStatus ttStatus = (taskTracker == null) ? null : taskTracker
					.getStatus();
			...
			if (state == TaskStatus.State.SUCCEEDED) {
				taskEvent = new TaskCompletionEvent(taskCompletionEventTracker, taskid,
						tip.idWithinJob(), status.getIsMap() && !tip.isJobCleanupTask()
			...
			// For a failed task update the JT datastructures.
				...
				failedTask(tip, taskid, status, taskTracker, wasRunning, wasComplete,
						wasAttemptRunning);
			...
			if (taskEvent != null) {
				...
				JobTrackerStatistics.TaskTrackerStat ttStat = jobtracker
						.getStatistics()
						.getTaskTrackerStat(tip.machineWhereTaskRan(taskid));
	...
	/**
	 * Returns map phase counters by summing over all map tasks in progress. This
	 * method returns true if counters are within limit or false.
	 */
	...
	/**
	 * Returns map phase counters by summing over all map tasks in progress. This
	 * method returns true if counters are within limits and false otherwise.
	 */
	 ...
	 * reduce counters. This method returns true if counters are within limits and
	 * false otherwise.
	 */
	...
	public synchronized Task obtainNewMapTask(TaskTrackerStatus tts,
			int clusterSize, int numUniqueHosts, int maxCacheLevel)
			throws IOException {
		...
		int target = findNewMapTask(tts, clusterSize, numUniqueHosts,
				maxCacheLevel, getStatus().mapProgress());
		if (target == -1) {
			return null;
		}
	 ...
	 * @return <code>true</code> if we can schedule off-switch, <code>false</code>
	 *         otherwise We check the number of missed opportunities for the job.
	...
	public boolean scheduleOffSwitch(int numTaskTrackers) {
		...
		long requiredSlots = Math.min((desiredMaps() - finishedMaps()),
				numTaskTrackers);
	...
	public Task obtainJobCleanupTask(TaskTrackerStatus tts, int clusterSize,
		...
		synchronized (this) {
			...
			if (result != null) {
				...
				if (jobFailed) {
					result
							.setJobCleanupTaskState(org.apache.hadoop.mapreduce.JobStatus.State.FAILED);
				} else if (jobKilled) {
					result
							.setJobCleanupTaskState(org.apache.hadoop.mapreduce.JobStatus.State.KILLED);
				} else {
					result
	 ...
	 * @return true/false
	 */
	public synchronized boolean canLaunchJobCleanupTask() {
	...
	public Task obtainJobSetupTask(TaskTrackerStatus tts, int clusterSize,
		...
		synchronized (this) {
			...
			TaskInProgress tip = findTaskFromList(setupTaskList, tts, numUniqueHosts,
					false);
			if (tip == null) {
				return null;
			}
			...
			if (result != null) {
				addRunningTaskToTIP(tip, result.getTaskID(), tts, true);
				runJobSetupTaskLocal(result, tts.getTrackerName());
			}
	...
	public void runJobSetupTaskLocal(Task t, String fakeTaskTracker)
			throws IOException {
		TaskAttemptID attemptID = t.getTaskID();
		JobContext jobContext = new JobContext(conf, this.jobId);

		TaskAttemptContext context = new TaskAttemptContext(this.conf, attemptID);
		try {
			org.apache.hadoop.mapreduce.OutputFormat<?, ?> outputFormat = ReflectionUtils
					.newInstance(context.getOutputFormatClass(), conf);
			org.apache.hadoop.mapreduce.OutputCommitter committer = outputFormat
					.getOutputCommitter(context);
			committer.setupJob(jobContext);

			TaskStatus ts = TaskStatus.createTaskStatus(true, attemptID, 1.0f, 1,
					State.SUCCEEDED, "", "setup", fakeTaskTracker, Phase.MAP,
					new Counters());

			this.updateTaskStatus(this.setup[0], ts);
	...
	public synchronized Task obtainNewReduceTask(TaskTrackerStatus tts,
		 ...
		 * for reducers is huge then we just fail the job and error out. The
		 * estimated size is divided by 2 since the resource estimator returns the
		 * amount of disk space the that the reduce will use (which is 2 times the
		 * input, space for merge + reduce input).
		 **/
		long estimatedReduceInputSize = resourceEstimator
				.getEstimatedReduceInputSize() / 2;
		if (((estimatedReduceInputSize) > reduce_input_limit)
				&& (reduce_input_limit > 0L)) {
			// make sure jobtracker lock is held
			LOG.info("Exceeded limit for reduce input size: Estimated:"
					+ estimatedReduceInputSize + " Limit: " + reduce_input_limit
					+ " Failing Job " + jobId);
			status.setFailureInfo("Job exceeded Reduce Input limit " + " Limit:  "
					+ reduce_input_limit + " Estimated: " + estimatedReduceInputSize);
	...
	static String convertTrackerNameToHostName(String trackerName) {
		...
		String trackerHostName = (indexOfColon == -1) ? trackerName : trackerName
	...
	public synchronized void reserveTaskTracker(TaskTracker taskTracker,
		...
		if (info == null) {
			...
			if (info.getNumSlots() != numSlots) {
				Enum<Counter> counter = (type == TaskType.MAP) ? Counter.FALLOW_SLOTS_MILLIS_MAPS
						: Counter.FALLOW_SLOTS_MILLIS_REDUCES;
				long fallowSlotMillis = (now - info.getTimestamp())
						* info.getNumSlots();
	...
	public synchronized void unreserveTaskTracker(TaskTracker taskTracker,
		...
		if (type == TaskType.MAP) {
			...
			jobtracker.getInstrumentation()
					.decReservedReduceSlots(info.getNumSlots());
		}
	...
	private synchronized void retireMap(TaskInProgress tip) {
		if (runningMapCache == null) {
			LOG
					.warn("Running cache for maps missing!! "
	 ...
	 *          a list of tips
	 * @param ttStatus
	 *          status of the tracker that has requested a tip
	 * @param avgProgress
	...
	private synchronized int findNewMapTask(final TaskTrackerStatus tts,
		...
		if (availSpace < outSize) {
			LOG.warn("No room for map task. Node " + tts.getHost() + " has "
					+ availSpace + " bytes free; but we expect map to take " + outSize);
		...
		if (hasSpeculativeMaps) {
			...
			if (node != null) {
				...
				for (int level = 0; level < maxLevel; ++level) {
					...
					if (cacheForLevel != null) {
						tip = findSpeculativeTask(cacheForLevel, tts, avgProgress,
								currentTime, level == 0);
						if (tip != null) {
							if (cacheForLevel.size() == 0) {
	...
	private synchronized int findNewReduceTask(TaskTrackerStatus tts,
		...
		if (availSpace < outSize) {
			LOG.warn("No room for reduce task. Node " + taskTracker + " has "
					+ availSpace + " bytes free; but we expect reduce input to take "
					+ outSize);
		...
		if (hasSpeculativeReduces) {
			tip = findSpeculativeTask(runningReduces, tts, avgProgress, jobtracker
					.getClock().getTime(), false);
			if (tip != null) {
				scheduleReduce(tip);
				return tip.getIdWithinJob();
			}
	 ...
	 * 
	 * @param tip
	 *          {@link TaskInProgress} to be metered which just completed, cannot
	 *          be <code>null</code>
	 * @param status
	 *          {@link TaskStatus} of the completed task, cannot be
	 *          <code>null</code>
	 */
	private void meterTaskAttempt(TaskInProgress tip, TaskStatus status) {
		Counter slotCounter = (tip.isMapTask()) ? Counter.SLOTS_MILLIS_MAPS
				: Counter.SLOTS_MILLIS_REDUCES;
		jobCounters.incrCounter(slotCounter, tip.getNumSlotsRequired()
	...
	public synchronized boolean completedTask(TaskInProgress tip,
		...
		// Update jobhistory
		TaskTrackerStatus ttStatus = this.jobtracker.getTaskTrackerStatus(status
				.getTaskTracker());
		String trackerHostname = jobtracker.getNode(ttStatus.getHost()).toString();
		String taskType = getTaskType(tip);
		if (status.getIsMap()) {
			JobHistory.MapAttempt.logStarted(status.getTaskID(), status
					.getStartTime(), status.getTaskTracker(), ttStatus.getHttpPort(),
					taskType);
			JobHistory.MapAttempt.logFinished(status.getTaskID(), status
					.getFinishTime(), trackerHostname, taskType, status.getStateString(),
					status.getCounters());
		} else {
			JobHistory.ReduceAttempt.logStarted(status.getTaskID(), status
					.getStartTime(), status.getTaskTracker(), ttStatus.getHttpPort(),
					taskType);
			JobHistory.ReduceAttempt.logFinished(status.getTaskID(), status
					.getShuffleFinishTime(), status.getSortFinishTime(), status
					.getFinishTime(), trackerHostname, taskType, status.getStateString(),
					status.getCounters());
		}
		JobHistory.Task.logFinished(tip.getTIPId(), taskType, tip
				.getExecFinishTime(), status.getCounters());
		...
		if (tip.isJobSetupTask()) {
			...
			LOG.info("job cleanup task finish.");
	...
	public void runCleanupTaskLocal(String taskTracker) {
		TaskTrackerStatus tts = jobtracker.getTaskTrackerStatus(taskTracker);

		try {
			if (this.canLaunchJobCleanupTask() == true) {
				Task cleanupTask = this.obtainJobCleanupTask(tts, jobtracker
						.getNumResolvedTaskTrackers(), this.jobtracker
						.getNumberOfUniqueHosts(), true);
				TaskAttemptID attemptID = cleanupTask.getTaskID();
				JobContext jobContext = new JobContext(conf, this.jobId);

				TaskAttemptContext context = new TaskAttemptContext(this.conf,
						attemptID);
				...
				if (jobFailed || jobKilled) {
					committer.abortJob(jobContext,
							jobFailed ? org.apache.hadoop.mapreduce.JobStatus.State.FAILED
									: org.apache.hadoop.mapreduce.JobStatus.State.KILLED);
				} else {
					committer.commitJob(jobContext);
				}

				TaskStatus fakeStatus = new MapTaskStatus(attemptID, 1.0f, 1,
						State.SUCCEEDED, "", "Successful", taskTracker, Phase.CLEANUP,
						new Counters());

				this.updateTaskStatus(cleanup[0], fakeStatus);
	...
	/**
	 * add up the counters and fail the job if it exceeds the counters. Make sure
	 * we do not recalculate the coutners after we fail the job. Currently this is
	 * taken care by terminateJob() since it does not calculate the counters.
	 */
	...
	private void jobComplete() {
		...
		if (this.status.getRunState() == JobStatus.RUNNING) {
			...
			JobHistory.JobInfo.logFinished(this.status.getJobID(), finishTime,
					...
					failedReduceTasks, mapCounters, reduceCounters, jobCounters);
	...
	private synchronized void terminateJob(int jobTerminationState) {
		if ((status.getRunState() == JobStatus.RUNNING)
			...
			if (jobTerminationState == JobStatus.FAILED) {
				jobtracker.getInstrumentation().failedJob(this.conf,
						this.status.getJobID());
			} else {
				jobtracker.getInstrumentation().killedJob(this.conf,
						this.status.getJobID());
			}
		}
	}
	...
	private void cancelReservedSlots() {
		...
		Set<TaskTracker> tm = new HashSet<TaskTracker>(trackersReservedForMaps
				.keySet());
		for (TaskTracker tt : tm) {
			tt.unreserveSlots(TaskType.MAP, this);
		}

		Set<TaskTracker> tr = new HashSet<TaskTracker>(trackersReservedForReduces
				.keySet());
	...
	private void failedTask(TaskInProgress tip, TaskAttemptID taskid,
			TaskStatus status, TaskTracker taskTracker, boolean wasRunning,
		...
		TaskTrackerStatus taskTrackerStatus = (taskTracker == null) ? null
	...
	public void failedTask(TaskInProgress tip, TaskAttemptID taskid,
		...
		TaskStatus status = TaskStatus.createTaskStatus(tip.isMapTask(), taskid,
				0.0f, tip.isMapTask() ? numSlotsPerMap : numSlotsPerReduce, state,
		...
		long startTime = oldStatus == null ? jobtracker.getClock().getTime()
		...
		if (wasComplete && !isComplete) { // mark a successful tip as failed
			String taskType = getTaskType(tip);
			JobHistory.Task.logFailed(tip.getTIPId(), taskType, tip
					.getExecFinishTime(), reason, taskid);
		}
	}
	...
	static class JobSummary {
		 ...
		 * 
		 * @param job
		 *          {@link JobInProgress} whose summary is to be logged, cannot be
		 *          <code>null</code>.
		 * @param cluster
		 *          {@link ClusterStatus} of the cluster on which the job was run,
		...
		public static void logJobSummary(JobInProgress job, ClusterStatus cluster) {
			...
			long mapSlotSeconds = (jobCounters.getCounter(Counter.SLOTS_MILLIS_MAPS) + jobCounters
					.getCounter(Counter.FALLOW_SLOTS_MILLIS_MAPS)) / 1000;
			long reduceSlotSeconds = (jobCounters
					.getCounter(Counter.SLOTS_MILLIS_REDUCES) + jobCounters
					.getCounter(Counter.FALLOW_SLOTS_MILLIS_REDUCES)) / 1000;

			SummaryBuilder summary = new SummaryBuilder()
					.add("jobId", job.getJobID()).add("submitTime", job.getStartTime())
					.add("launchTime", job.getLaunchTime()).add(
							getTaskLaunchTimesSummary(job)).add("finishTime",
							job.getFinishTime()).add("numMaps",
							job.getTasks(TaskType.MAP).length).add("numSlotsPerMap",
							job.getNumSlotsPerMap()).add("numReduces",
							job.getTasks(TaskType.REDUCE).length).add("numSlotsPerReduce",
							job.getNumSlotsPerReduce()).add("user", profile.getUser()).add(
							"queue", profile.getQueueName()).add("status",
							JobStatus.getJobRunState(status.getRunState())).add(
							"mapSlotSeconds", mapSlotSeconds).add("reduceSlotsSeconds",
							reduceSlotSeconds).add("clusterMapCapacity",
							cluster.getMaxMapTasks()).add("clusterReduceCapacity",
							cluster.getMaxReduceTasks());
